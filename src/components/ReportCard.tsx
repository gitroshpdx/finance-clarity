...
const ReportCard = ({ slug, title, teaser, readTime, topic, publishedAt, index = 0 }: ReportCardProps) => {
  ...
  return (
    <Link to={`/report/${slug}`}>
      ...
      >
      <div className="glass-card rounded-2xl overflow-hidden transition-all duration-500 group-hover:shadow-card-hover group-hover:border-primary/20 h-full flex flex-col">
        {/* Thumbnail / Visual */}
        <div className={`relative h-48 bg-gradient-to-br ${gradientClass} overflow-hidden`}>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <motion.div
              className="w-24 h-24 rounded-3xl bg-background/20 backdrop-blur-md flex items-center justify-center border border-white/10 shadow-2xl"
          ...
          <motion.span
            ...
            className="absolute top-4 left-4 px-3 py-1.5 text-xs font-semibold rounded-full bg-background/90 backdrop-blur-sm text-foreground shadow-lg"
          >
            {topic}
          </motion.span>
        </div>
